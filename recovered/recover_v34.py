#!/usr/bin/env python3
"""Route and verify volume 34 publisher PDFs by saved SHA-256 identity."""
...
DEFAULT_CORPUS = ROOT / "corpus-v34"
...
def load_volume_34_destinations(
    inventory: Path = DEFAULT_INVENTORY,
    corpus: Path = DEFAULT_CORPUS,
) -> dict[str, Path]:
    with inventory.open(newline="", encoding="utf-8-sig") as handle:
        rows = [row for row in csv.DictReader(handle) if row["volume"] == "34"]
    if len(rows) != 29:
        raise ValueError(f"expected 29 volume 34 inventory rows, found {len(rows)}")
    destinations = {row["sha256"]: corpus / row["internal_filename"] for row in rows}
    if len(destinations) != 29:
        raise ValueError("volume 34 inventory contains duplicate member hashes")
    return destinations
...
def verify_corpus(
    ...
    if require_complete and missing:
        raise ValueError(f"missing {len(missing)} volume 34 PDFs")
...
def main() -> None:
    args = parse_args()
    destinations = load_volume_34_destinations()
    if args.downloads:
