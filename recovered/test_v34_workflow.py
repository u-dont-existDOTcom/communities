#!/usr/bin/env python3
"""Regression tests for the volume 34 recovery and discovery boundary."""
...
from recovered.recover_v34 import route_downloads


ROOT = Path(__file__).resolve().parent
INVENTORY = ROOT / "COMMUNITIES-SOURCE-INVENTORY.csv"
RANKING = ROOT / "V34-DISCOVERY-RANKING.csv"
KEYWORD_CONTEXTS = ROOT / "v34-keyword-contexts.txt"
...
class Volume34RecoveryTest(unittest.TestCase):
    def test_route_downloads_uses_saved_hash_instead_of_browser_name(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            destination = root / "corpus" / "vol34" / "iss1" / "001-front-matter.pdf"
...
class Volume34WorkflowTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        inventory_rows = load_csv(INVENTORY)
        cls.expected_rows = [row for row in inventory_rows if row["volume"] == "34"]
        cls.expected_ids = {row["record_id"] for row in cls.expected_rows}

    def test_inventory_boundary_is_29_pdfs_in_two_issues(self) -> None:
        self.assertEqual(len(self.expected_rows), 29)
        self.assertEqual(sum(row["issue"] == "1" for row in self.expected_rows), 18)
        self.assertEqual(sum(row["issue"] == "2" for row in self.expected_rows), 11)

    def test_volume_34_discovery_outputs_cover_every_inventory_row(self) -> None:
