#!/usr/bin/env python3
"""Route and verify volume 36 publisher PDFs by saved SHA-256 identity."""
...
DEFAULT_CORPUS = ROOT / "corpus-v36"
...
def load_volume_36_destinations(
    inventory: Path = DEFAULT_INVENTORY,
    corpus: Path = DEFAULT_CORPUS,
) -> dict[str, Path]:
    with inventory.open(newline="", encoding="utf-8-sig") as handle:
        rows = [row for row in csv.DictReader(handle) if row["volume"] == "36"]
    if len(rows) != 21:
        raise ValueError(f"expected 21 volume 36 inventory rows, found {len(rows)}")
    destinations = {row["sha256"]: corpus / row["internal_filename"] for row in rows}
    if len(destinations) != 21:
        raise ValueError("volume 36 inventory contains duplicate member hashes")
    return destinations
...
def verify_corpus(
    ...
    if require_complete and missing:
        raise ValueError(f"missing {len(missing)} volume 36 PDFs")
...
def main() -> None:
    args = parse_args()
    destinations = load_volume_36_destinations()
    if args.downloads:
