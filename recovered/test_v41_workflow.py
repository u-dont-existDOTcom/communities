#!/usr/bin/env python3
"""Regression tests for the volume 41 recovery boundary."""
...
from recovered.recover_v41 import load_volume_41_destinations, route_downloads


ROOT = Path(__file__).resolve().parent
INVENTORY = ROOT / "COMMUNITIES-SOURCE-INVENTORY.csv"
RANKING = ROOT / "V41-DISCOVERY-RANKING.csv"
KEYWORD_CONTEXTS = ROOT / "v41-keyword-contexts.txt"
...
class Volume41RecoveryTest(unittest.TestCase):
    def test_recovery_script_runs_from_repository_root(self) -> None:
        result = subprocess.run(
            [sys.executable, str(ROOT / "recover_v41.py"), "--allow-incomplete"],
            cwd=ROOT.parent,
            capture_output=True,
            text=True,
            check=False,
    ...
    def test_inventory_loader_maps_exact_volume_41_boundary(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            corpus = Path(directory) / "corpus-v41"
            ...
            self.assertTrue(
                all(path.is_relative_to(corpus / "vol41") for path in destinations.values())
            )
    ...
    def test_route_downloads_uses_saved_hash_instead_of_browser_name(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            destination = root / "corpus" / "vol41" / "iss1" / "001-front-matter.pdf"
...
class Volume41WorkflowTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        inventory_rows = load_csv(INVENTORY)
        cls.expected_rows = [row for row in inventory_rows if row["volume"] == "41"]
        cls.expected_ids = {row["record_id"] for row in cls.expected_rows}

    def test_inventory_boundary_is_20_pdfs_in_issue_1(self) -> None:
        self.assertEqual(len(self.expected_rows), 20)
        self.assertTrue(all(row["issue"] == "1" for row in self.expected_rows))

    def test_volume_41_discovery_outputs_cover_every_inventory_row(self) -> None:
    ...
    def test_ranking_preserves_term_and_process_families(self) -> None:
        ...
        previous = ROOT / "discover_v40.py"
        current = ROOT / "discover_v41.py"
        self.assertEqual(
    ...
    def test_discovery_classifies_functional_metadata(self) -> None:
        ...
        expected_metadata = {"M-0004", "M-0019", "M-0020", "M-0021"}
        self.assertEqual(
            {
                record_id
        ...
        self.assertEqual(rows["M-0004"]["kind"], "back_matter")
        self.assertEqual(rows["M-0019"]["kind"], "editorial")
        self.assertEqual(rows["M-0020"]["kind"], "table_of_contents")
        self.assertEqual(rows["M-0021"]["kind"], "front_matter")
        self.assertEqual(
