#!/usr/bin/env python3
"""Route and verify volume 35 publisher PDFs by saved SHA-256 identity."""
...
DEFAULT_CORPUS = ROOT / "corpus-v35"
...
def load_volume_35_destinations(
    inventory: Path = DEFAULT_INVENTORY,
    corpus: Path = DEFAULT_CORPUS,
) -> dict[str, Path]:
    with inventory.open(newline="", encoding="utf-8-sig") as handle:
        rows = [row for row in csv.DictReader(handle) if row["volume"] == "35"]
    if len(rows) != 26:
        raise ValueError(f"expected 26 volume 35 inventory rows, found {len(rows)}")
    destinations = {row["sha256"]: corpus / row["internal_filename"] for row in rows}
    if len(destinations) != 26:
        raise ValueError("volume 35 inventory contains duplicate member hashes")
    return destinations
...
def verify_corpus(
    ...
    if require_complete and missing:
        raise ValueError(f"missing {len(missing)} volume 35 PDFs")
...
def main() -> None:
    args = parse_args()
    destinations = load_volume_35_destinations()
    if args.downloads:
