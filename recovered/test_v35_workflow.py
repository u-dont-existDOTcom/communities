#!/usr/bin/env python3
"""Regression tests for the volume 35 recovery and discovery boundary."""
...
from recovered.recover_v35 import route_downloads


ROOT = Path(__file__).resolve().parent
INVENTORY = ROOT / "COMMUNITIES-SOURCE-INVENTORY.csv"
RANKING = ROOT / "V35-DISCOVERY-RANKING.csv"
KEYWORD_CONTEXTS = ROOT / "v35-keyword-contexts.txt"
...
class Volume35RecoveryTest(unittest.TestCase):
...
class Volume35WorkflowTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        inventory_rows = load_csv(INVENTORY)
        cls.expected_rows = [row for row in inventory_rows if row["volume"] == "35"]
    ...
    def test_inventory_boundary_is_26_pdfs_in_two_issues(self) -> None:
        ...
        self.assertEqual(sum(row["issue"] == "1" for row in self.expected_rows), 12)
        self.assertEqual(sum(row["issue"] == "2" for row in self.expected_rows), 14)

    def test_volume_35_discovery_outputs_cover_every_inventory_row(self) -> None:
