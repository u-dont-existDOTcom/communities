...
CORPUS = ROOT / "corpus-v26-v28"
RANKING = ROOT / "V26-V28-DISCOVERY-RANKING.csv"
CONTEXTS = ROOT / "v26-v28-keyword-contexts.txt"
CHILD_CONTEXTS = ROOT / "v26-v28-child-danger-contexts.txt"
...
with INVENTORY.open(newline="", encoding="utf-8-sig") as handle:
    source_rows = [
        ...
        if row.get("volume", "").isdigit() and 26 <= int(row["volume"]) <= 28
    ]
assert len(source_rows) == 82
...
ranking_rows.sort(key=lambda item: (item["kind"] in METADATA_KINDS, -int(item["score"]), item["record_id"]))
...
print(
    f"ranked={len(ranking_rows)} nonmetadata={sum(row['kind'] not in METADATA_KINDS for row in ranking_rows)} "
