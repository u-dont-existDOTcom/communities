#!/usr/bin/env python3
"""Regression tests for the volume 42 recovery boundary."""
...
from recovered.recover_v42 import load_volume_42_destinations, route_downloads


ROOT = Path(__file__).resolve().parent
INVENTORY = ROOT / "COMMUNITIES-SOURCE-INVENTORY.csv"
RANKING = ROOT / "V42-DISCOVERY-RANKING.csv"
KEYWORD_CONTEXTS = ROOT / "v42-keyword-contexts.txt"
...
class Volume42RecoveryTest(unittest.TestCase):
    def test_recovery_script_runs_from_repository_root(self) -> None:
        result = subprocess.run(
            [sys.executable, str(ROOT / "recover_v42.py"), "--allow-incomplete"],
            cwd=ROOT.parent,
            capture_output=True,
            text=True,
            check=False,
        ...
        self.assertEqual(sum(map(int, receipt.groups())), 24)

    def test_inventory_loader_maps_exact_volume_42_boundary(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            corpus = Path(directory) / "corpus-v42"

            destinations = load_volume_42_destinations(INVENTORY, corpus)

            self.assertEqual(len(destinations), 24)
            self.assertEqual(
            ...
            self.assertTrue(
                all(path.is_relative_to(corpus / "vol42") for path in destinations.values())
            )
    ...
    def test_route_downloads_uses_saved_hash_instead_of_browser_name(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            destination = root / "corpus" / "vol42" / "iss2" / "001-front-matter.pdf"
...
class Volume42WorkflowTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        inventory_rows = load_csv(INVENTORY)
        cls.expected_rows = [row for row in inventory_rows if row["volume"] == "42"]
        cls.expected_ids = {row["record_id"] for row in cls.expected_rows}

    def test_inventory_boundary_is_24_pdfs_split_across_two_issues(self) -> None:
        self.assertEqual(len(self.expected_rows), 24)
        self.assertEqual(
            Counter(row["issue"] for row in self.expected_rows),
            Counter({"1": 12, "2": 12}),
        )

    def test_volume_42_discovery_outputs_cover_every_inventory_row(self) -> None:
    ...
    def test_ranking_preserves_term_and_process_families(self) -> None:
        ...
        previous = ROOT / "discover_v41.py"
        current = ROOT / "discover_v42.py"
        self.assertEqual(
