#!/usr/bin/env python3
"""Fresh structural verification for the completed volume 39 checkpoint."""
...
volume_rows = [row for row in inventory_rows if row["volume"] == "39"]
assert len(volume_rows) == 23
assert Counter(row["issue"] for row in volume_rows) == Counter({"1": 11, "2": 12})
assert {row["record_id"] for row in volume_rows} == {
    f"M-{number:04d}" for number in range(956, 979)
}
...
assert Counter(row["research_status"] for row in volume_rows) == Counter({
    "metadata triaged": 8,
    "contextual close read; no distinct finding": 12,
...
for row in volume_rows:
    ...
    assert row["local_path"] == f"recovered/corpus-v39/{row['internal_filename']}"
    assert row["text_path"] == f"recovered/corpus-v39/{row['internal_filename'][:-4]}.txt"
...
assert "through volume 39" in gap_text
assert (
    "F-031, F-048, F-064, F-076, F-090, F-100, F-105, F-111, "
    "F-115, F-118, F-121, F-125, F-131, F-138, F-142"
) in gap_text
...
for required in [
    ...
    "298 metadata-triaged",
    "**142 findings** (`F-001` through `F-142`)",
    "Volumes **40-45** have not been processed: **138 journal PDFs**",
    "volume **40: 9 PDFs**",
]:
    assert required in state, required
assert "Do not repeat volumes 1-39" in state

report = REPORT.read_text(encoding="utf-8")
for required in [
    "All **23 PDFs** in volume 39 were processed",
    "**15** substantive close reads",
    "**0** additional articles or reviews left at title-and-keyword triage",
    "**8** front-matter, contents, editorial, and back-matter metadata triages",
    "**4 new findings, F-139 through F-142**",
    "All 23 publisher PDFs matched the pre-existing archive-member SHA-256 values",
...
for required in [
    ...
    "recovered/COMMUNITIES-V39-RESEARCH-REPORT.md",
    "recovered/corpus-v39/",
...
assert not any(path.startswith("recovered/corpus-v39/") for path in tracked)
assert not any(path.endswith((".pdf", ".zip", ".png")) for path in tracked)
assert not any(
    "v39-keyword-contexts" in path or "v39-child-danger-contexts" in path
    for path in tracked
)

volume_40_rows = [row for row in inventory_rows if row["volume"] == "40"]
assert len(volume_40_rows) == 9
assert Counter(row["issue"] for row in volume_40_rows) == Counter({"1": 9})
assert all(row["research_status"] == "not processed" for row in volume_40_rows)
remaining_rows = [
    row
    for row in inventory_rows
    if row["record_type"] == "archive_pdf"
    and row["volume"].isdigit()
    and int(row["volume"]) >= 40
]
assert len(remaining_rows) == 138

print("PASS volume39_publisher_member_hashes=23 pages_verified=23 text_nonempty=23 shared_archive_reverified=0")
print("PASS inventory_dispositions promoted_sources=3 contextual=12 title=0 metadata=8")
print("PASS ledger_rows=142 sequential_ids=142 new_statuses=B2,C1,F1")
print("PASS gap_rows=18 classes=B8,C7,D3 references_valid=1")
print("PASS discovery_rows=23 substantive=15 metadata=8 child_candidate_files=4")
print("PASS report_close_read_rows=15 state_boundary=volumes_1_39")
print("PASS next_boundary=volume40 pdfs=9 issue1=9 remaining_40_45=138")
