#!/usr/bin/env python3
"""Regression-test Unit E updater idempotency and verification."""
...
def main() -> None:
    with tempfile.TemporaryDirectory(prefix="communities-fair-separation-") as temp:
        copy = Path(temp) / "repo"
        shutil.copytree(
            REPOSITORY,
            copy,
        ...
        updater = recovered / "update_adjacent_fair_separation.py"
        verifier = recovered / "verify_adjacent_fair_separation.py"

        run(updater, copy)
        first = digest(copy)
        run(updater, copy)
        second = digest(copy)
        assert first == second, "Unit E updater must be byte-idempotent"
        run(verifier, copy)

    print("adjacent fair separation workflow regression: PASS")
