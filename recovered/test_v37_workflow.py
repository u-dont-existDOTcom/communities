#!/usr/bin/env python3
"""Regression tests for the volume 37 recovery and discovery boundary."""
...
from recovered.recover_v37 import route_downloads


ROOT = Path(__file__).resolve().parent
INVENTORY = ROOT / "COMMUNITIES-SOURCE-INVENTORY.csv"
RANKING = ROOT / "V37-DISCOVERY-RANKING.csv"
KEYWORD_CONTEXTS = ROOT / "v37-keyword-contexts.txt"
...
class Volume37RecoveryTest(unittest.TestCase):
...
class Volume37WorkflowTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        inventory_rows = load_csv(INVENTORY)
        cls.expected_rows = [row for row in inventory_rows if row["volume"] == "37"]
        cls.expected_ids = {row["record_id"] for row in cls.expected_rows}

    def test_inventory_boundary_is_26_pdfs_in_two_issues(self) -> None:
        self.assertEqual(len(self.expected_rows), 26)
        self.assertEqual(sum(row["issue"] == "1" for row in self.expected_rows), 16)
        self.assertEqual(sum(row["issue"] == "2" for row in self.expected_rows), 10)

    def test_volume_37_discovery_outputs_cover_every_inventory_row(self) -> None:
