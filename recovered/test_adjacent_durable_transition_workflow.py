#!/usr/bin/env python3
"""Regression-test Unit C updater idempotency and verification."""
...
def main() -> None:
    with tempfile.TemporaryDirectory(prefix="communities-durable-transition-") as temp:
        copy = Path(temp) / "repo"
        shutil.copytree(
            REPOSITORY,
            copy,
        ...
        updater = recovered / "update_adjacent_durable_transition.py"
        verifier = recovered / "verify_adjacent_durable_transition.py"

        run(updater, copy)
        first = digest(copy)
        run(updater, copy)
        second = digest(copy)
        assert first == second, "Unit C updater must be byte-idempotent"
        run(verifier, copy)

    print("adjacent durable treatment workflow regression: PASS")
