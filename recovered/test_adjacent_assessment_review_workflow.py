#!/usr/bin/env python3
"""Regression-test Unit B updater idempotency and verification."""
...
def main() -> None:
    with tempfile.TemporaryDirectory(prefix="communities-assessment-review-") as temp:
        copy = Path(temp) / "repo"
        shutil.copytree(
            REPOSITORY,
            copy,
        ...
        updater = recovered / "update_adjacent_assessment_review.py"
        verifier = recovered / "verify_adjacent_assessment_review.py"

        run(updater, copy)
        first = digest(copy)
        run(updater, copy)
        second = digest(copy)
        assert first == second, "Unit B updater must be byte-idempotent"
        run(verifier, copy)

    print("adjacent assessment and review workflow regression: PASS")
