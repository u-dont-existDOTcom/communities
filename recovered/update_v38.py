#!/usr/bin/env python3
"""Apply the completed volume 38 checkpoint to cumulative research artifacts."""
...
def update_inventory() -> None:
    ...
    for row in rows:
        ...
        if row["record_type"] != "archive_pdf" or row["volume"] != "38":
            continue
        ...
        row["local_path"] = f"recovered/corpus-v38/{row['internal_filename']}"
        row["text_path"] = f"recovered/corpus-v38/{row['internal_filename'][:-4]}.txt"
        dispositions[disposition] += 1

    assert seen == {f"M-{number:04d}" for number in range(936, 956)}
    assert dispositions == Counter({"metadata": 7, "contextual": 7, "promoted": 6})
    archive_row = next(row for row in rows if row["record_id"] == ARCHIVE_RECORD_ID)
    for field, value in ARCHIVE_EXPECTED.items():
...
def update_state() -> None:
    ...
    replacements = [
        ("volumes **1-37**", "volumes **1-38**", "state completed boundary"),
        (
        ...
        ("Do not repeat volumes 1-37.", "Do not repeat volumes 1-38.", "state resume boundary"),
        (
...
def update_readme() -> None:
    ...
    replacements = [
        ("Volumes **1-37** complete", "Volumes **1-38** complete", "README boundary"),
        ("**803** journal PDFs triaged", "**823** journal PDFs triaged", "README PDF count"),
        ("**313** relevant or contextual close reads", "**326** relevant or contextual close reads", "README close reads"),
        ("**131** evidence findings (`F-001` through `F-131`)", "**138** evidence findings (`F-001` through `F-138`)", "README findings"),
        (
        ...
        ("`recovered/corpus-v37/`", "`recovered/corpus-v38/`", "README corpus path"),
        ("python recovered/test_v37_workflow.py", "python recovered/test_v38_workflow.py", "README tests"),
        ("python recovered/verify_v37.py", "python recovered/verify_v38.py", "README verifier"),
        ("all 26 PDF hashes", "all 20 PDF hashes", "README verified PDFs"),
        ("the volume-38 boundary", "the volume-39 boundary", "README next-boundary check"),
    ]
    for old, new, label in replacements:
        text = replace_once_or_confirm(text, old, new, label)
...
def main() -> None:
    ...
    update_state()
    update_readme()
    print("updated volume38 findings=7 promoted_sources=6 contextual=7 metadata=7")
