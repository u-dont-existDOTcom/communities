...
- Regression suite: `python recovered/test_adjacent_official_correction_workflow.py` (or the current bounded-unit successor)
- Full source verification: `python recovered/verify_adjacent_official_correction.py`; run source-dependent predecessor checks only when their exact local corpora are restored
